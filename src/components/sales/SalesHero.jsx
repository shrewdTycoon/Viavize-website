...
const outcomes = [
  { label: 'Equip reps', dot: 'bg-turmeric-bright' },
  { label: 'Shorten the cycle', dot: 'bg-cyan' },
  { label: 'Win more deals', dot: 'bg-cyan' },
]

/* A sales kit of assets, plus a deal advancing through the pipeline */
function SalesKit() {
  ...
  const assets = ['Pitch deck', 'One-pager', 'Case study', 'Battlecard', 'Email templates']
  const stages = [
    { label: 'Discovery', done: true },
    { label: 'Demo', done: true },
    ...
    <div className="relative w-full max-w-[400px] mx-auto" aria-hidden="true">
      ...
      >
        <motion.div {...float(0)}
          ...
          style={{ background: 'linear-gradient(135deg, rgba(40,30,4,0.5) 0%, rgba(0,24,56,0.96) 62%)' }}
        >
          ...
          <div className="px-4 py-4 flex flex-col gap-2">
            {assets.map((a, i) => (
              ...
              >
                <span className="w-6 h-6 rounded-md bg-white/[0.05] border border-white/[0.08]
                  flex items-center justify-center flex-shrink-0">
                  <svg viewBox="0 0 24 24" fill="none" className="w-3 h-3">
                    <path d="M14 2H6a2 2 0 00-2 2v16a2 2 0 002 2h12a2 2 0 002-2V8l-6-6z" stroke="#8FA6BD" strokeWidth="2" strokeLinejoin="round"/>
                  </svg>
                </span>
                <span className="flex-1 text-[11.5px] font-medium text-white/80">{a}</span>
      ...
      {/* Pipeline progress */}
      <motion.div {...fade(1.7)}
        className="relative z-10 mt-3 rounded-xl border border-[rgba(0,193,255,0.18)] px-4 py-3.5
          shadow-[0_8px_28px_rgba(0,14,33,0.5)]"
        style={{ background: 'linear-gradient(135deg, rgba(0,40,78,0.95) 0%, rgba(0,26,60,0.95) 100%)' }}
      >
        <span className="text-[9px] font-semibold uppercase tracking-[1px] text-cyan block mb-2.5">
          Deal stage
        </span>
            ...
            <div key={s.label} className="flex items-center flex-1 last:flex-none">
              <div className="flex flex-col items-center gap-1">
                <span className={`w-2.5 h-2.5 rounded-full
                  ${s.done ? 'bg-cyan' : 'bg-white/15 border border-white/25'}`} />
                <span className={`text-[7.5px] font-medium ${s.done ? 'text-white/70' : 'text-white/40'}`}>
                  {s.label}
                </span>
              </div>
              {i < stages.length - 1 && (
                <span className={`h-px flex-1 mx-1 ${s.done ? 'bg-cyan/40' : 'bg-white/10'}`} />
              )}
            </div>
          ))}
        </div>
      ...
      {/* Floating chips */}
      <motion.div {...fade(2.2)} className="absolute -left-3 top-[16%] z-20">
        <motion.div {...float(0.5, 4, 5)}
          className="px-2.5 py-1.5 rounded-lg bg-turmeric/10 border border-turmeric/25">
          <span className="text-[9px] font-semibold text-turmeric-bright">On-message</span>
        </motion.div>
      </motion.div>
      <motion.div {...fade(2.35)} className="absolute -right-2 top-[44%] z-20">
        <motion.div {...float(1.5, 4, 5.5)}
          className="px-2.5 py-1.5 rounded-lg bg-cyan/10 border border-cyan/25">
          <span className="text-[9px] font-semibold text-cyan">Rep-ready</span>
        </motion.div>
      </motion.div>
    </div>
  )
          ...
          <div>
            ...
            >
              <span className="block text-turmeric">
                <Words text="Give sales the material," />
              </span>
              <span className="block text-cyan">
                <Words text="and the story to sell it." />
              </span>
            </motion.h1>
            ...
            >
              Viavize builds the decks, one-pagers, case studies, and messaging
              your sales team actually uses &mdash; so every rep tells the same,
              strongest version of your story.
            </motion.p>

            <motion.p {...fade(0.58)}
              className="text-[17px] leading-relaxed text-white/85 font-medium max-w-[520px] mb-8"
            >
              Marketing and sales working from the same script.
            </motion.p>
