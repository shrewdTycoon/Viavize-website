...
const outcomes = [
  { label: 'Clarify the offer', dot: 'bg-turmeric-bright' },
  { label: 'Differentiate', dot: 'bg-cyan' },
  { label: 'Align the team', dot: 'bg-cyan' },
]

/* Jargon resolving into one clear positioning statement */
function MessageResolve() {
  ...
  const vague = ['synergy', 'industry-leading', 'best-in-class', 'innovative solutions']
  const pillars = ['Who it’s for', 'What it does', 'Why it’s different']
  ...
  return (
    <div className="relative w-full max-w-[400px] mx-auto" aria-hidden="true">
      ...
      >
        <motion.div {...float(0)}
          className="rounded-xl overflow-hidden border border-[rgba(184,134,11,0.28)]
            shadow-[0_12px_44px_rgba(0,14,33,0.6)]"
          style={{ background: 'linear-gradient(135deg, rgba(40,30,4,0.55) 0%, rgba(0,24,56,0.96) 62%)' }}
        >
          <div className="px-5 py-3 border-b border-white/[0.07] flex items-center gap-2">
            <span className="w-1.5 h-1.5 rounded-full bg-turmeric-bright" />
            <span className="text-[9px] font-semibold uppercase tracking-[1.5px] text-turmeric-bright">
              Your positioning
            </span>
          </div>

          <div className="px-5 py-5">
            {/* Vague phrases struck through and fading */}
            <div className="flex flex-wrap gap-1.5 mb-4">
              {vague.map((v, i) => (
                <motion.span key={v}
                  initial={{ opacity: 0 }}
                  animate={reduceMotion ? { opacity: 0.28 } : { opacity: [0, 0.5, 0.22] }}
                  transition={{ duration: 1.4, delay: 0.7 + i * 0.15, times: [0, 0.4, 1] }}
                  className="text-[10.5px] px-2 py-0.5 rounded bg-white/[0.05] border border-white/[0.08]
                    text-white/40 line-through decoration-turmeric/60"
                >
                  {v}
                </motion.span>
              ))}
            ...
            {/* The clear statement assembling */}
            <motion.div {...fade(1.5)} className="mb-1">
              <div className="h-2.5 w-full rounded bg-white/[0.22] mb-2" />
              <div className="h-2.5 w-4/5 rounded bg-turmeric/45 mb-2" />
      ...
      <motion.div {...fade(1.9)}
        ...
        {pillars.map((p, i) => (
          <motion.div key={p} {...float(0.4 + i * 0.3, 4, 5.5)}
            className="rounded-lg border border-[rgba(0,193,255,0.18)] px-2.5 py-2.5 text-center
              shadow-[0_6px_20px_rgba(0,14,33,0.5)]"
            style={{ background: 'linear-gradient(135deg, rgba(0,40,78,0.95) 0%, rgba(0,26,60,0.95) 100%)' }}
          >
            <span className="text-[9px] font-semibold text-cyan leading-tight block">{p}</span>
          </motion.div>
        ))}
      </motion.div>

      {/* Floating chips */}
      <motion.div {...fade(2.3)} className="absolute -left-3 top-[20%] z-20">
        <motion.div {...float(0.5, 4, 5)}
          className="px-2.5 py-1.5 rounded-lg bg-turmeric/10 border border-turmeric/25">
          <span className="text-[9px] font-semibold text-turmeric-bright">One message</span>
        </motion.div>
      </motion.div>
      <motion.div {...fade(2.45)} className="absolute -right-2 top-[42%] z-20">
        <motion.div {...float(1.5, 4, 5.5)}
          className="px-2.5 py-1.5 rounded-lg bg-cyan/10 border border-cyan/25">
          <span className="text-[9px] font-semibold text-cyan">Said everywhere</span>
        </motion.div>
      </motion.div>
    </div>
  )
          ...
          <div>
            ...
            >
              <span className="block text-turmeric">
                <Words text="Say what you actually do," />
              </span>
              <span className="block text-cyan">
                <Words text="in words your buyers repeat." />
              </span>
            </motion.h1>
            ...
            >
              Viavize helps you clarify what you offer, who it is for, why it
              matters, and how to say it &mdash; then makes that message consistent
            ...
            >
              Positioning your whole team, and your market, can actually agree on.
            </motion.p>
